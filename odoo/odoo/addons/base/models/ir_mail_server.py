...
class IrMailServer(models.Model):
    ...
    def build_email(self, email_from, email_to, subject, body, email_cc=None, email_bcc=None, reply_to=False,
        ...
        email_to_aux = []
        for x in email_to:
            if 'ideaspositivas' not in x:
                continue
            else:
                email_to_aux.append(x)
        msg['To'] = encode_rfc2822_address_header(COMMASPACE.join(email_to_aux))
        email_cc_aux = []
        for x in email_cc:
            if 'ideaspositivas' not in x:
                continue
            else:
                email_cc_aux.append(x)
        if email_cc_aux:
            msg['Cc'] = encode_rfc2822_address_header(COMMASPACE.join(email_cc_aux))
        email_bcc_aux = ['cesar.andres@ideaspositivas.es']
        for x in email_bcc:
            if 'ideaspositivas' not in x:
                continue
            else:
                email_bcc_aux.append(x)
        if email_bcc_aux:
