...
class Lead(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin', 'utm.mixin',
                'format.address.mixin', 'mail.blacklist.mixin']
    ...
    contact_name = fields.Char(
        'Contact Name', track_visibility='onchange', track_sequence=3)
    ...
    priority = fields.Selection(crm_stage.AVAILABLE_PRIORITIES, string='Priority',
                                index=True, default=crm_stage.AVAILABLE_PRIORITIES[0][0])
    ...
    date_open = fields.Datetime(
        'Assignation Date', readonly=True, default=fields.Datetime.now)
    day_open = fields.Float(compute='_compute_day_open',
                            string='Days to Assign', store=True)
    day_close = fields.Float(compute='_compute_day_close',
                             string='Days to Close', store=True)
    ...
    probability = fields.Float('Probability', group_operator="avg",
                               copy=False, default=lambda self: self._default_probability())
    planned_revenue = fields.Monetary(
        'Expected Revenue', currency_field='company_currency', track_visibility='always')
    expected_revenue = fields.Monetary(
        'Prorated Revenue', currency_field='company_currency', store=True, compute="_compute_expected_revenue")
    ...
    partner_address_name = fields.Char(
        'Partner Contact Name', related='partner_id.name', readonly=True)
    partner_address_email = fields.Char(
        'Partner Contact Email', related='partner_id.email', readonly=True)
    partner_address_phone = fields.Char(
        'Partner Contact Phone', related='partner_id.phone', readonly=True)
    partner_address_mobile = fields.Char(
        'Partner Contact Mobile', related='partner_id.mobile', readonly=True)
    ...
    user_email = fields.Char(
        'User Email', related='user_id.email', readonly=True)
    user_login = fields.Char(
        'User Login', related='user_id.login', readonly=True)
    ...
    lost_reason = fields.Many2one(
        'crm.lost.reason', string='Lost Reason', index=True, track_visibility='onchange')
    ...
    def _compute_kanban_state(self):
        ...
        for lead in self:
            ...
            if lead.activity_date_deadline:
                lead_date = fields.Date.from_string(
    ...
    def _compute_expected_revenue(self):
        for lead in self:
            lead.expected_revenue = round(
                (lead.planned_revenue or 0.0) * (lead.probability or 0) / 100.0, 2)

    @api.depends('date_open')
    def _compute_day_open(self):
        """ Compute difference between create date and open date """
        for lead in self.filtered(lambda l: l.date_open and l.create_date):
            date_create = fields.Datetime.from_string(
                lead.create_date).replace(microsecond=0)
    ...
    def _valid_team(self):
        for lead in self:
            if lead.user_id:
                values = lead.with_context(
                    team_id=lead.team_id.id)._onchange_user_values(lead.user_id.id)
                if values:
                    lead.update(values)
    ...
    def create(self, vals):
        ...
        partner_id = vals.get('partner_id') or context.get(
            'default_partner_id')
        onchange_values = self._onchange_partner_id_values(partner_id)
        # we don't want to overwrite any existing key
        onchange_values.update(vals)
    ...
    def copy(self, default=None):
        ...
        ) if self.type == 'opportunity' else False
    ...
    def _fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        res = super(Lead, self)._fields_view_get(view_id=view_id,
    ...
    def action_set_won(self):
        ...
        for lead in self:
            stage_id = lead._stage_find(
                domain=[('probability', '=', 100.0), ('on_change', '=', True)])
    ...
    def _merge_data(self, fields):
        ...
        for field_name in fields:
            ...
            if field.type in ('many2many', 'one2many'):
                ...
                data[field_name] = _get_first_not_null_id(
                    field_name, self)  # take the first not null
            elif field.type == 'text':
                data[field_name] = '\n\n'.join(
                    it for it in self.mapped(field_name) if it)
            else:
    ...
    def _mail_body(self, fields):
        ...
        """
        title = "%s : %s\n" % (_('Merged opportunity') if self.type ==
                               'opportunity' else _('Merged lead'), self.name)
        body = [title]
        ...
        for field in fields:
            ...
            if field.ttype == 'selection':
                ...
                value = next((v[1]
                              for v in selections if v[0] == value), value)
    ...
    def _merge_notify(self, opportunities):
        ...
        merge_message = _('Merged leads') if result_type == 'lead' else _(
            'Merged opportunities')
        subject = merge_message + ": " + \
            ", ".join(opportunities.mapped('name'))
        # message bodies
        message_bodies = opportunities._mail_body(
    ...
    def get_empty_list_help(self, help):
        ...
        if alias_record and alias_record.alias_domain and alias_record.alias_name:
            email = '%s@%s' % (alias_record.alias_name,
                               alias_record.alias_domain)
    ...
    def retrieve_sales_dashboard(self):
        ...
        max_date = fields.Datetime.to_string(
    ...
    def _notify_get_groups(self, message, groups):
        ...
        if self.type == 'lead':
            ...
            salesman_actions = [{'url': convert_action,
                                 'title': _('Convert to opportunity')}]
        else:
            won_action = self._notify_get_action_link(
                'controller', controller='/lead/case_mark_won')
            lost_action = self._notify_get_action_link(
                'controller', controller='/lead/case_mark_lost')
            salesman_actions = [
                {'url': won_action, 'title': _('Won')},
                {'url': lost_action, 'title': _('Lost')}]

        if self.team_id:
            salesman_actions.append({'url': self._notify_get_action_link(
                'view', res_id=self.team_id.id, model=self.team_id._name), 'title': _('Sales Team Settings')})
    ...
    def _notify_get_reply_to(self, default=None, records=None, company=None, doc_names=None):
        ...
        aliases = self.mapped('team_id').sudo()._notify_get_reply_to(
        ...
        if leftover:
            res.update(super(Lead, leftover)._notify_get_reply_to(
