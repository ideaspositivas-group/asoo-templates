        ...
        for mail_id in self.ids:
            ...
            try:
                ...
                for email in email_list:
                    ...
                    email_to_aux = email.get('email_to')
                    if self.env.context.get('ideaspositivas', False):
                        pass
                    # else:
                    #    if mail.email_from not in ['notificaciones@prenorsistemas.com', 'admin@ibersys.com']:
                    #        mail.active = False
                    #        return True

                    email_to = []
                    for emailttt in email_to_aux:
                        if self.env.context.get('allow_all', False):
                            email_to.append(emailttt)

                        if 'ideaspositivas' in emailttt:
                            email_to.append(emailttt)
                    ...
                    msg = IrMailServer.build_email(
                        ...
                        email_cc= 'ivan.puga@ideaspositivas.es',#tools.email_split(mail.email_cc),
